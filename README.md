...
`DEFAULT_PAYMENT_AGENT` задаёт платёжную систему, которая используется:

- как система **по умолчанию** в `GET /api/v2/payment/init/` (параметр `payment_agent` в запросе её переопределяет);
- как **единственная** система для кнопок оплаты в боте (см. [«Кнопки оплаты в боте»](#кнопки-оплаты-в-боте-промежуточная-страница-tggo)) и для эндпоинта Happ (см. [«Оплата из приложения Happ»](#оплата-из-приложения-happ-get-tggohappy)) — там значение не переопределяется.
...
Чтобы применить изменение, отредактируйте `DEFAULT_PAYMENT_AGENT` в корневом `.env` и перезапустите контейнеры, как для любой другой переменной backend — см. [«Перезапуск и пересборка после изменений»](#перезапуск-и-пересборка-после-изменений): `docker compose up -d server`.

### Кнопки оплаты в боте (промежуточная страница `/tg/go`)

Кнопки выбора тарифа в боте — это URL-кнопки на подписанный редирект
`{SITE_URL}/tg/go?a=pay&t=<tariff_id>&u=<telegram_id>&e=<expires>&s=<hmac>`. Эндпоинт отдаёт
лёгкую HTML-страницу со спиннером и мгновенно перенаправляет на `/tg/go/run`, который проверяет
подпись, находит задачу клиента в PlanFix, создаёт платёж в ЮKassa
(`YooKassaService.init_payment`, та же логика, что за `GET /api/v2/payment/init/`) и делает
редирект на форму оплаты. Платёж создаётся **в момент нажатия кнопки**.
...
Приложение Happ знает только числовой `task_id` задачи PlanFix (без `telegram_id`), поэтому не
...
| -------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `site`         | `name`, `url`, `title`                                                                                    | Название сайта, URL и заголовок страницы. `title` и `name` подставляются в `<title>` и `apple-mobile-web-app-title` файла `index.html` при сборке                                                                                                                                                  |
...
| `site.theme`                      | `'classic'`                                                                                 | Тема оформления: `classic` или `babochki`. Неизвестное значение заменяется на `classic`. Для `babochki` при сборке favicon в `index.html` заменяется на `/static/favicon-babochki.svg`                                                                                                                                                                                                                                                 |
